...
/** ---- Tiny in-memory rate limiter (per sender) as a fallback ---- */
...
// persist across HMR/test runs; avoid `any`
...
export async function GET(req: NextRequest) {
  // Robust URL extraction: prefer NextRequest.nextUrl, else fall back safely
  let url: URL;
  const anyReq = req as unknown as { nextUrl?: URL; url?: string };
  if (anyReq?.nextUrl instanceof URL) {
    url = anyReq.nextUrl;
  } else {
    const raw =
      typeof anyReq?.url === 'string' && anyReq.url.length ? anyReq.url : '/api/lobby/messages';
    url = raw.startsWith('http') ? new URL(raw) : new URL(`http://localhost${raw}`);
  }
  ...
  // tests expect >= 2 messages from the mock chain on happy path
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed.success) {
      // Return the specific address error if present
      const msg =
    ...
    if (error) {
      // Map mock/DB RL signal to the exact text/status the test expects
